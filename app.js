...
function renderFeed() {
  ...
  window.MARTBOOK_DATA.posts.forEach(p => {
    const card = document.createElement('div');
    card.className = 'post-card';
    card.innerHTML = `<h3>${p.user}</h3><p>${p.content}</p>`;
...
document.getElementById('add-post-btn').onclick = () => {
  ...
  if(!content) return;
  window.MARTBOOK_DATA.posts.unshift({user: currentUser.name, content});
